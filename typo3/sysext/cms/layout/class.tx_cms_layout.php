 ...
 *
 * $Id: class.tx_cms_layout.php 7753 2010-05-29 21:28:00Z lolli $
	...
	function tt_content_drawItem($row, $isRTE=FALSE)	{
		...
		if($row['header']) {
			...
			$outHeader=  ($row['date'] ? htmlspecialchars($this->itemLabels['date'].' '.t3lib_BEfunc::date($row['date'])).'<br />':'').
					$this->infoGif($infoArr). '<strong>' . $this->linkEditContent($this->renderText($row['header']), $row) . $hiddenHeaderNote . '</strong><br />';
		}
