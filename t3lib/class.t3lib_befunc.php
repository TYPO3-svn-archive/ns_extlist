 ...
 *
 * $Id: class.t3lib_befunc.php 7751 2010-05-29 21:26:12Z lolli $
...
final class t3lib_BEfunc {
    ...
    if (is_array($TCA_DESCR[$table]) && is_array($TCA_DESCR[$table]['columns'][$field]) && (isset($BE_USER->uc['edit_showFieldHelp']) || $force)) {
			...
			return '<a class="typo3-csh-link" href="'.htmlspecialchars($onClick).'" >' . t3lib_iconWorks::getSpriteIcon('actions-system-help-open', array('class' => 'typo3-csh-icon')) . $text.'</a>';
		}
	}
	...
	public static function cshItem($table, $field, $BACK_PATH, $wrap = '', $onlyIconMode = FALSE, $styleAttrib = '') {
		global $TCA_DESCR, $LANG, $BE_USER, $TYPO3_CONF_VARS;
        ...
        */
        if(is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_befunc.php']['cshItemTempClass'])) {            
            foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_befunc.php']['cshItemTempClass'] as $classData) {
              $hookObject = t3lib_div::getUserObj($classData);
              $output = $hookObject->cshItemHook($table, $field, $BACK_PATH, $wrap = '', $onlyIconMode = FALSE, $styleAttrib = '', $this);
            }
        }
