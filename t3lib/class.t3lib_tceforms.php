 ...
 *
 * $Id: class.t3lib_tceforms.php 7793 2010-05-31 15:37:06Z ohader $
...
class t3lib_TCEforms	{
	...
	function getSingleField($table,$field,$row,$altName='',$palette=0,$extra='',$pal=0)	{
		global $TCA,$BE_USER, $TYPO3_CONF_VARS; 
		...
		if (	is_array($PA['fieldConf']) &&
				...
				if (in_array($field,$this->hiddenFieldListArr))	{
					...
					if ($PA['fieldConf']['config']['form_type']=='user' && $PA['fieldConf']['config']['noTableWrapping'])	{
						...
						$out=array(
							...
							'TABLE'=>$table,
							'ITEM'=>$item,
							'HELP_ICON' => t3lib_BEfunc::cshItem($table, $field,$GLOBALS['BACK_PATH']) //edited  by sameera
						);
          ...
          //extention can add custom field template as they need 	       
          $customTemplate = '';
						
          /**
           * @hook			tceformsTempClass : confiug to add custom field template
           * @date			2010-08-13
           * @request		Nuwan Sameera  <nuwan28@gmail.com>
           */          
          if(is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsIntoTemplate']) && $row['customedit']) {            
                foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsIntoTemplate'] as $classData) {
                  $hookObject = t3lib_div::getUserObj($classData);
                  $customTemplate = $hookObject->customIntoTemplate($this);
                }
          }             
            // String:       
            $out = $this->intoTemplate($out, $customTemplate);
    ...
    global $TYPO3_CONF_VARS;
		...
		$item .='<input type="text" id="' . $inputId . '" class="' . implode(' ', $cssClasses) . '" name="'.$PA['itemFormElName'].'_hr" value="" style="' . $cssStyle . '" maxlength="'.$mLgd.'" onchange="'.htmlspecialchars($iOnChange).'"'.$PA['onFocus'].' />';	// This is the EDITABLE form field.		
		   ...
		   /**
		    * @hook	 tceformsTempClass :  @date	# 2010-08-13 # @request	Nuwan Sameera  <nuwan28@gmail.com>
		    */
		    if(is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTempClass']) && $row['customedit']) {            
		        foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTempClass'] as $classData) {
	        		// if field is date value
	        		if (isset($config['checkbox']))	{
	        			$PA['itemFormElValue'] = (($PA['itemFormElValue'])? date("j-n-Y", $PA['itemFormElValue']): "" );
	        		}
		          	$hookObject = t3lib_div::getUserObj($classData);
		          	$item = $hookObject->customGetSingleField_typeInput($inputId,$cssClasses, $mLgd,$PA,$PA['itemFormElName'].'_hr', $cssStyle,$field,  $this);
		        }
		    }
	...
	function getSingleField_typeCheck($table,$field,$row,&$PA)	{
	   ...
	   /**
	    * @hook			tceformsTypeCheck :  cretae custom check box for quick edit panel
	    * @date			2010-08-13
	    * @request		Nuwan Sameera  <nuwan28@gmail.com>
	    */
	    if(is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTypeCheck']) && $row['customedit']) {            
	        foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTypeCheck'] as $classData) {
	          $hookObject = t3lib_div::getUserObj($classData);
	          $item = $hookObject->customGetSingleField_typeCheck($this->insertDefStyle('check'), $cBName, $cBID, $PA['itemFormElName'], $disabled, $c, $thisValue, $this);
	        }
	    }
	...
	function getSingleField_typeSelect_single($table,$field,$row,&$PA,$config,$selItems,$nMV_label)	{		
		...
		$itemSelect = '<select' . $selectedStyle . ' id="' . uniqid('tceforms-select-') . '" name="' . $PA['itemFormElName'] . '"' .
	   ...
	   /**
	    * @hook			tceformsTypeSelect :  cretae custom check box for quick edit panel
	    * @date			2010-08-13
	    * @request		Nuwan Sameera  <nuwan28@gmail.com>
	    */
	    
	    $selectIconOption = ($config['iconsInOptionTags'] ? $this->insertDefStyle('select', 'icon-select') : $this->insertDefStyle('select')) ;
	    
	    if(is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTypeSelect']) && $row['customedit']) {            
	        foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tceforms.php']['tceformsTypeSelect'] as $classData) {	        	
	          $hookObject = t3lib_div::getUserObj($classData);
	          $itemSelect = $hookObject->customGetSingleField_typeSelect($selectedStyle, uniqid('tceforms-select-'), $selectIconOption, $PA['itemFormElName'], $disabled, $this);
	        }
	    }
